...
public class Cliente {
		...
		public static int executeSync(DataBase db){
			...
			{			
				...
				//rp3.auna.models.Cliente.deleteAll(db, Contract.Cliente.TABLE_NAME);
				//rp3.auna.models.ClienteDireccion.deleteAll(db, Contract.ClienteDireccion.TABLE_NAME);
				//rp3.auna.models.Cliente.ClientExt.deleteAll(db, Contract.ClientExt.TABLE_NAME);
				...
				for(int i=0; i < types.length(); i++){
					...
					try {
						...
						rp3.auna.models.Cliente cl = rp3.auna.models.Cliente.getClienteIDServer(db, type.getLong("IdCliente"), false);
						if(cl == null)
							cl = new rp3.auna.models.Cliente();
                        if(type.getString("Estado").equalsIgnoreCase(Contants.ESTADO_ELIMINADO)) {
							rp3.auna.models.Cliente.delete(db, cl);
						}
                        else
                        {
                            ...
                            rp3.auna.models.ClienteDireccion.deleteClienteDireccionIdCliente(db, cl.getIdCliente());

                            for (int j = 0; j < strs.length(); j++) {
                                JSONObject str = strs.getJSONObject(j);
                                rp3.auna.models.ClienteDireccion clienteDir = new rp3.auna.models.ClienteDireccion();

                                if(str.getString("Estado").equalsIgnoreCase(Contants.ESTADO_ELIMINADO))
                                {
                                    rp3.auna.models.ClienteDireccion toDelete = rp3.auna.models.ClienteDireccion.getClienteDireccionIdDireccion(db, str.getLong("IdCliente"), str.getInt("IdClienteDireccion"));
                                    if(toDelete != null)
                                    {
                                        rp3.auna.models.ClienteDireccion.delete(db, toDelete);
                                    }
                                }
                                else {
                                    ...
                                    rp3.auna.models.ClienteDireccion.insert(db, clienteDir);
                                }
                            }

                            strs = type.getJSONArray("ClienteContactos");

                            //rp3.auna.models.Contacto.deleteContactoIdCliente(db, cl.getIdCliente());

                            for (int j = 0; j < strs.length(); j++) {
                                JSONObject str = strs.getJSONObject(j);
								rp3.auna.models.Contacto clienteCont = rp3.auna.models.Contacto.getContactoId(db, str.getLong("IdClienteContacto"), str.getLong("IdCliente"));

                                if(str.getString("Estado").equalsIgnoreCase(Contants.ESTADO_ELIMINADO))
                                {
                                    if(clienteCont.getID() != 0)
                                    {
                                        rp3.auna.models.Contacto.delete(db, clienteCont);
                                    }
                                }
                                else {
									...
									if(clienteCont.getID() != 0)
										rp3.auna.models.Contacto.update(db, clienteCont);
									else
										rp3.auna.models.Contacto.insert(db, clienteCont);
                                }
                            }
                            if (rp3.auna.models.Cliente.getClienteIDServer(db, cl.getIdCliente(), false) == null) {
                                rp3.auna.models.Cliente.insert(db, cl);
                            } else {
                                rp3.auna.models.Cliente.update(db, cl);
                            }
                        }
    ...
    public static int executeSyncDeletes(DataBase db){
        ...
        try
        {
            ...
            rp3.auna.models.Cliente.deleteClientes(db, listIds);
        }finally{
            webService.close();
        }
		...
		public static int executeSyncCreate(DataBase db, long cliente){
			...
			int id = 0;
			rp3.auna.models.Cliente cl = rp3.auna.models.Cliente.getClienteID(db, cliente, true);
				...
				try {
					...
					for(int s = 0; s < responseArray.length(); s ++)
					{
						...
						for(int i = 0; i < contactos.length(); i++)
						{
							...
							rp3.auna.models.Contacto cliCon = cl.getContactos().get(i);
							cliCon.setIdCliente(id);
							cliCon.setIdContacto(jObjectCont.getInt("IdServer"));
							rp3.auna.models.Contacto.update(db, cliCon);
						}
					}
			...
			{				
				...
				for(int r = 0; r < cl.getContactos().size(); r ++)
				{
					...
					if(foto != null)
					{
						...
						webService.invokeWebService();
						String nom_foto = webService.getStringResponse();
						rp3.auna.models.Contacto ct = cl.getContactos().get(r);
						nom_foto = nom_foto.replaceAll("\"", "");
						ct.setURLFoto(nom_foto);
						rp3.auna.models.Contacto.update(db, ct);
					}
				}
			} catch (HttpResponseException e) {
				if(e.getStatusCode() == HttpConnection.HTTP_STATUS_UNAUTHORIZED)
			...
			{			
				...
				try {
					...
					for(int s = 0; s < responseArray.length(); s ++)
					{
						...
						JSONObject type = responseArray.getJSONObject(s);
						for(rp3.auna.models.Cliente cl : clientes)
						{
		...
		public static int executeSyncInserts(DataBase db){
					...
					for(int s = 0; s < responseArray.length(); s ++)
					{
						...
						for(rp3.auna.models.Cliente cl : clientes)
						{
							if(cl.getID() == type.getInt("IdInterno"))
							{
								...
								cl.setPendiente(false);
								rp3.auna.models.Cliente.update(db, cl);
								List<rp3.auna.models.Agenda> agds = rp3.auna.models.Agenda.getAgendaClienteInterno(db, cl.getID());
								for(rp3.auna.models.Agenda agd : agds)
								{
									agd.setIdCliente(id);
									agd.setIdClienteDireccion(1);
									rp3.auna.models.Agenda.update(db, agd);
								}
			...
			{
				rp3.auna.models.Cliente cl = clientesConId.get(i);
				webService = new WebService("MartketForce","SetFotos");			
