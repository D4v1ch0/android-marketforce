...
public class Rutas {
		...
		public static int executeSync(DataBase db, Long inicio, Long fin, boolean inList){
            ...
            fechaUlt.setTime(SyncAudit.getLastSyncDate(rp3.auna.sync.SyncAdapter.SYNC_TYPE_ACT_AGENDA, SyncAdapter.SYNC_EVENT_SUCCESS));
			...
			{			
				...
				//rp3.auna.models.Agenda.deleteAll(db, Contract.Agenda.TABLE_NAME);
				//rp3.auna.models.Agenda.AgendaExt.deleteAll(db, Contract.AgendaExt.TABLE_NAME);
				//rp3.auna.models.AgendaTarea.deleteAll(db, Contract.AgendaTarea.TABLE_NAME);
				...
				for(int i=0; i < types.length(); i++){
					...
					try {
						...
						rp3.auna.models.Agenda agenda = rp3.auna.models.Agenda.getAgendaServer(db, (int) type.getLong("IdAgenda"));
						if(agenda == null) {
							if (!type.isNull("Origen") && type.getString("Origen").equalsIgnoreCase("M")) {
								agenda = rp3.auna.models.Agenda.getAgendaServerByTicks(db, Convert.getDateFromDotNetTicks(type.getLong("FechaCreacionTicks")).getTime());
								if (agenda == null)
									agenda = new Agenda();
							} else
								agenda = new Agenda();
						}
						if(agenda.isEnviado() || agenda.getID() == 0) {
							if (type.getString("EstadoAgenda").equalsIgnoreCase(Contants.ESTADO_ELIMINADO)) {
								rp3.auna.models.Agenda.deleteAgendaIdServer(db, (int) type.getLong("IdAgenda"));
							} else {
								...
								//rp3.auna.models.AgendaTarea.deleteTareas(db, agenda.getIdRuta(), agenda.getIdAgenda());
								...
								for (int j = 0; j < strs.length(); j++) {
									...
									if (!existe) {
										rp3.auna.models.AgendaTarea agendaTarea = new rp3.auna.models.AgendaTarea();
										...
										rp3.auna.models.AgendaTarea.insert(db, agendaTarea);
									}
								}
							}
						}
