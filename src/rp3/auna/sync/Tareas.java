...
public class Tareas {
	...
	public static int executeSync(DataBase db){
        ...
        fechaUlt.setTime(SyncAudit.getLastSyncDate(rp3.auna.sync.SyncAdapter.SYNC_TYPE_TODO, SyncAdapter.SYNC_EVENT_SUCCESS));
		...
		{			
			...
			//rp3.auna.models.Tarea.deleteAll(db, Contract.Tareas.TABLE_NAME);
			//rp3.auna.models.Actividad.deleteAll(db, Contract.Actividades.TABLE_NAME);
			//rp3.auna.models.AgendaTareaOpciones.deleteAll(db, Contract.AgendaTareaOpciones.TABLE_NAME);
			//rp3.auna.models.AgendaTareaActividades.deleteAll(db, Contract.AgendaTareaActividades.TABLE_NAME);
			//rp3.auna.models.AgendaTareaOpciones.deleteAll(db, Contract.AgendaTareaOpciones.TABLE_NAME);
			...
			for(int i=0; i < types.length(); i++){
				...
				try {					
						...
						rp3.auna.models.Tarea tarea = new rp3.auna.models.Tarea();

                        if(type.getString("Estado").equalsIgnoreCase(Contants.ESTADO_ELIMINADO))
                        {
                            rp3.auna.models.Tarea toDelete = rp3.auna.models.Tarea.getTareaId(db,type.getInt("IdTarea"));
                            if(toDelete != null)
                            {
                                rp3.auna.models.Tarea.delete(db, toDelete);
                            }
                        }
                        else {
                            ...
                            rp3.auna.models.Tarea tareaUpdate = rp3.auna.models.Tarea.getTareaId(db, tarea.getIdTarea());

                            if(tareaUpdate != null)
                                rp3.auna.models.Tarea.update(db, tarea);
                            else
                                rp3.auna.models.Tarea.insert(db, tarea);

                            rp3.auna.models.Actividad.deleteFromTareas(db, tarea.getIdTarea());

                            JSONArray strs = type.getJSONArray("TareaActividades");

                            for (int j = 0; j < strs.length(); j++) {
                                JSONObject str = strs.getJSONObject(j);
                                rp3.auna.models.Actividad actividad = new rp3.auna.models.Actividad();
                                ...
                                rp3.auna.models.Actividad.insert(db, actividad);

                                rp3.auna.models.AgendaTareaOpciones.deleteOpciones(db, tarea.getIdTarea(), actividad.getIdTareaActividad());

                                JSONArray opcs = str.getJSONArray("TareaOpciones");

                                for (int k = 0; k < opcs.length(); k++) {
                                    JSONObject opc = opcs.getJSONObject(k);
                                    rp3.auna.models.AgendaTareaOpciones opcion = new rp3.auna.models.AgendaTareaOpciones();
                                    ...
                                    rp3.auna.models.AgendaTareaOpciones.insert(db, opcion);
                                }
                            }
                        }
					}
    ...
    public static int executeSyncTareaActualizacion(DataBase db, int id){
        ...
        try
        {
            ...
            try {
                ...
                for (int j = 0; j < strs.length(); j++) {
                    JSONObject str = strs.getJSONObject(j);
                    Campo campo = new rp3.auna.models.Campo();
                    ...
                    rp3.auna.models.Campo.insert(db, campo);
