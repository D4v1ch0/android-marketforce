...
public class Cliente {
		...
		public static int executeSync(DataBase db){
			...
			{			
				...
				long fecha = rp3.util.Convert.getDotNetTicksFromDate(SyncAudit.getLastSyncDate(rp3.berlin.sync.SyncAdapter.SYNC_TYPE_CLIENTE_UPDATE, SyncAdapter.SYNC_EVENT_SUCCESS));
				...
				for(int i=0; i < types.length(); i++){
					...
					try {
						...
						rp3.berlin.models.Cliente cl = rp3.berlin.models.Cliente.getClienteIDServer(db, type.getLong("IdCliente"), false);
						if(cl == null)
							cl = new rp3.berlin.models.Cliente();
                        if(type.getString("Estado").equalsIgnoreCase(Contants.ESTADO_ELIMINADO)) {
							rp3.berlin.models.Cliente.delete(db, cl);
						}
                        else
                        {
                            ...
                            rp3.berlin.models.ClienteDireccion.deleteClienteDireccionIdCliente(db, cl.getIdCliente());

                            for (int j = 0; j < strs.length(); j++) {
                                JSONObject str = strs.getJSONObject(j);
                                rp3.berlin.models.ClienteDireccion clienteDir = new rp3.berlin.models.ClienteDireccion();

                                if(str.getString("Estado").equalsIgnoreCase(Contants.ESTADO_ELIMINADO))
                                {
                                    rp3.berlin.models.ClienteDireccion toDelete = rp3.berlin.models.ClienteDireccion.getClienteDireccionIdDireccion(db, str.getLong("IdCliente"), str.getInt("IdClienteDireccion"));
                                    if(toDelete != null)
                                    {
                                        rp3.berlin.models.ClienteDireccion.delete(db, toDelete);
                                    }
                                }
                                else {
                                    ...
                                    rp3.berlin.models.ClienteDireccion.insert(db, clienteDir);
                                }
                            }
                            ...
                            for (int j = 0; j < strs.length(); j++) {
                                JSONObject str = strs.getJSONObject(j);
								rp3.berlin.models.Contacto clienteCont = rp3.berlin.models.Contacto.getContactoId(db, str.getLong("IdClienteContacto"), str.getLong("IdCliente"));

                                if(str.getString("Estado").equalsIgnoreCase(Contants.ESTADO_ELIMINADO))
                                {
                                    if(clienteCont.getID() != 0)
                                    {
                                        rp3.berlin.models.Contacto.delete(db, clienteCont);
                                    }
                                }
                                else {
									...
									if(clienteCont.getID() != 0)
										rp3.berlin.models.Contacto.update(db, clienteCont);
									else
										rp3.berlin.models.Contacto.insert(db, clienteCont);
                                }
                            }
                            if (rp3.berlin.models.Cliente.getClienteIDServer(db, cl.getIdCliente(), false) == null) {
                                rp3.berlin.models.Cliente.insert(db, cl);
                            } else {
                                rp3.berlin.models.Cliente.update(db, cl);
                            }
                        }
    ...
    public static int executeSyncDeletes(DataBase db){
        ...
        try
        {
            ...
            rp3.berlin.models.Cliente.deleteClientes(db, listIds);
        }finally{
            webService.close();
        }
		...
		public static int executeSyncCreate(DataBase db, long cliente){
			...
			int id = 0;
			rp3.berlin.models.Cliente cl = rp3.berlin.models.Cliente.getClienteID(db, cliente, true);
				...
				try {
					...
					for(int s = 0; s < responseArray.length(); s ++)
					{
						...
						for(int i = 0; i < contactos.length(); i++)
						{
							...
							rp3.berlin.models.Contacto cliCon = cl.getContactos().get(i);
							cliCon.setIdCliente(id);
							cliCon.setIdContacto(jObjectCont.getInt("IdServer"));
							rp3.berlin.models.Contacto.update(db, cliCon);
						}
					}
			...
			{				
				...
				for(int r = 0; r < cl.getContactos().size(); r ++)
				{
					...
					if(foto != null)
					{
						...
						webService.invokeWebService();
						String nom_foto = webService.getStringResponse();
						rp3.berlin.models.Contacto ct = cl.getContactos().get(r);
						nom_foto = nom_foto.replaceAll("\"", "");
						ct.setURLFoto(nom_foto);
						rp3.berlin.models.Contacto.update(db, ct);
					}
				}
			} catch (HttpResponseException e) {
				if(e.getStatusCode() == HttpConnection.HTTP_STATUS_UNAUTHORIZED)
		...
		public static int executeSyncPendientes(DataBase db){
			...
			List<rp3.berlin.models.Cliente> clientes = rp3.berlin.models.Cliente.getClientePendientes(db, true);
			if(clientes.size() == 0)
				return SyncAdapter.SYNC_EVENT_SUCCESS;
			JSONObject jObject = new JSONObject();
			JSONArray jArray = new JSONArray();
			...
			{
				rp3.berlin.models.Cliente cl = clientes.get(s);
				try
				{
					jObject = new JSONObject();
			...
			{			
				...
				try {
					...
					for(int s = 0; s < responseArray.length(); s ++)
					{
						...
						JSONObject type = responseArray.getJSONObject(s);
						for(rp3.berlin.models.Cliente cl : clientes)
						{
		...
		public static int executeSyncInserts(DataBase db){
			...
			int id = 0;
			List<rp3.berlin.models.Cliente> clientes = rp3.berlin.models.Cliente.getClienteInserts(db, true);
			List<rp3.berlin.models.Cliente> clientesConId = new ArrayList<rp3.berlin.models.Cliente>();
			if(clientes.size() == 0)
				return SyncAdapter.SYNC_EVENT_SUCCESS;
			JSONObject jObject = new JSONObject();
			JSONArray jArray = new JSONArray();
					...
					for(int s = 0; s < responseArray.length(); s ++)
					{
						...
						for(rp3.berlin.models.Cliente cl : clientes)
						{
							if(cl.getID() == type.getInt("IdInterno"))
							{
								...
								cl.setPendiente(false);
								rp3.berlin.models.Cliente.update(db, cl);
								List<rp3.berlin.models.Agenda> agds = rp3.berlin.models.Agenda.getAgendaClienteInterno(db, cl.getID());
								for(rp3.berlin.models.Agenda agd : agds)
								{
									agd.setIdCliente(id);
									agd.setIdClienteDireccion(1);
									rp3.berlin.models.Agenda.update(db, agd);
								}
			...
			{
				rp3.berlin.models.Cliente cl = clientesConId.get(i);
				webService = new WebService("MartketForce","SetFotos");			
	...
	public static Bundle executeSyncCompras(String cliente){
		...
		try
		{
			...
			JSONArray types = webService.getJSONArrayResponse();
			if(types != null)
				resp.putString(ComprasClienteFragment.ARG_CLIENTE, types.toString());
			else
				resp.putString(ComprasClienteFragment.ARG_CLIENTE, "");
			resp.putInt(rp3.content.SyncAdapter.ARG_SYNC_TYPE,rp3.content.SyncAdapter.SYNC_EVENT_SUCCESS);
		}finally{
			webService.close();
		}
