...
public class Rutas {
		...
		public static int executeSync(DataBase db, Long inicio, Long fin, boolean inList){
            ...
            fechaUlt.setTime(SyncAudit.getLastSyncDate(rp3.berlin.sync.SyncAdapter.SYNC_TYPE_ACT_AGENDA, SyncAdapter.SYNC_EVENT_SUCCESS));
			...
			{			
				...
				for(int i=0; i < types.length(); i++){
					...
					try {
						...
						rp3.berlin.models.Agenda agenda = rp3.berlin.models.Agenda.getAgendaServer(db, (int) type.getLong("IdAgenda"));
						if(agenda == null) {
							if (!type.isNull("Origen") && type.getString("Origen").equalsIgnoreCase("M")) {
								agenda = rp3.berlin.models.Agenda.getAgendaServerByTicks(db, Convert.getDateFromDotNetTicks(type.getLong("FechaCreacionTicks")).getTime());
								if (agenda == null)
									agenda = new Agenda();
							} else
								agenda = new Agenda();
						...
						if(agenda.isEnviado() || agenda.getID() == 0) {
							if (type.getString("EstadoAgenda").equalsIgnoreCase(Contants.ESTADO_ELIMINADO)) {
								rp3.berlin.models.Agenda.deleteAgendaIdServer(db, (int) type.getLong("IdAgenda"));
							} else {
								...
								if (agenda.getID() == 0) {
									rp3.berlin.models.Agenda.insert(db, agenda);
								} else {
								...
								for (int j = 0; j < strs.length(); j++) {
									...
									if (!existe) {
										rp3.berlin.models.AgendaTarea agendaTarea = new rp3.berlin.models.AgendaTarea();
										...
										rp3.berlin.models.AgendaTarea.insert(db, agendaTarea);
									}
								}
							}
						}
