...
import rp3.berlin.marcaciones.PermisoActivity;
...
public class GCMReceiver extends GcmListenerService {
    ...
    public void onMessageReceived(String from, Bundle data) {
            ...
            else
            {
                if(type.equalsIgnoreCase(NOTIFICATION_TYPE_MARCACION)) {
                    try {
                        ...
                        if (ultimaMarcacion == null || rp3.berlin.models.marcacion.Marcacion.getMarcacionesPendientes(db).size() == 0) {
                            toSpeech = title;
                            if(getApplicationContext() == null)
                                Utils.ErrorToFile("Context is null - " + Calendar.getInstance().getTime().toString());
                        ...
                        rp3.berlin.sync.Marcaciones.executeSync(db);
                    }
                    catch(Exception ex)
                    {
                        Utils.ErrorToFile(ex);
        ...
        else
        {
            ...
            {
                rp3.berlin.sync.Agente.executeSyncLog();
            }
        }
